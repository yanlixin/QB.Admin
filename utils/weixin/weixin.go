...
func (wx *Weixin) GetUserList() ([]string, error) {
	...
	for len(result) != total {
		reply, err := sendGetRequest(fmt.Sprintf("%s/user/get?next_openid=%s&access_token=", weixinHost, next_openid), wx.tokenChan)
		if err != nil {
			return nil, err
		}
		var userData struct {
		...
		if err := json.Unmarshal(reply, &userData); err != nil {
			return nil, err
		}
		total = userData.Total
		result = append(result, userData.Data.OpenID...)
		next_openid = userData.Next_OpenID
	}
